...
public class PetCirCleInsideAdapter<T> extends CommonAdapter<T>{
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		ImageView imageview_petcircle_icon = viewHolder.getView(R.id.imageview_petcircle_icon);
		GlideUtil.loadRoundImg(mContext, inside.avatar, imageview_petcircle_icon,
		...
		if (!TextUtils.isEmpty(inside.memberIcon)) {
			if (inside.duty==1||inside.duty==2) {
				...
				viewHolder.getView(R.id.imageview_petcircle_tag).setVisibility(View.VISIBLE);
				GlideUtil.loadImg(mContext,inside.memberIcon, (ImageView)viewHolder.getView(R.id.imageview_petcircle_tag),0);
			}
		}else {
			viewHolder.getView(R.id.imageview_petcircle_tag).setVisibility(View.GONE);
		}
		...
		if (inside.smallImgsList.size()>0) {
			...
			if (inside.smallImgsList.size()>3) {
				...
				viewHolder.setBackgroundNormal(mContext,R.id.one_img, inside.smallImgsList.get(0), R.drawable.icon_production_default);
				viewHolder.setBackgroundNormal(mContext,R.id.two_img, inside.smallImgsList.get(1), R.drawable.icon_production_default);
				viewHolder.setBackgroundNormal(mContext,R.id.thr_img, inside.smallImgsList.get(2), R.drawable.icon_production_default);
			}else {
		...
		viewHolder.getView(R.id.imageview_petcircle_icon).setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (Utils.checkLogin(mContext)) {
					...
					goToNext(LoginNewActivity.class,0,0, "");
				}
			}
		});
	}
