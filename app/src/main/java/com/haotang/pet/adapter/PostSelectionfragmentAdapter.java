...
public class PostSelectionfragmentAdapter<T> extends CommonAdapter<T> {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        ImageView sriv_item_postselectionfragment_userimg = viewHolder
        ...
        RelativeLayout.LayoutParams mPreviewParams = (RelativeLayout.LayoutParams) rl_item_postselectionfragment_picorvideo
                .getLayoutParams();
        mPreviewParams.width = ScreenUtil.getScreenWidth(mContext);
        mPreviewParams.height = ScreenUtil.getScreenWidth(mContext) * 69 / 75;
        ...
        if (postsBean != null) {
            ...
            GlideUtil.loadRoundImg(mContext, avatar, sriv_item_postselectionfragment_userimg, R.drawable.icon_self, 5);
            if (imgs != null && imgs.size() > 0) {
                ...
                if (imgUrl != null && !TextUtils.isEmpty(imgUrl)) {
                    GlideUtil.loadImg(mContext, imgUrl,
            ...
            if (giftUsers != null && giftUsers.size() > 0) {
                for (int i = 0; i < giftUsers.size() - 1; i++) {
                    for (int j = giftUsers.size() - 1; j > i; j--) {
                        if (giftUsers.get(j).getId() == giftUsers.get(i).getId()) {
                            giftUsers.remove(j);
                        }
                    }
                }
    ...
    private void initPlayer(ImageView iv_item_postselectionfragment_img,
        ...
        if (coverImgs != null && coverImgs.size() > 0) {
            ...
            if (coverImg != null && !TextUtils.isEmpty(coverImg)) {
                GlideUtil.loadImg(mContext, coverImg,
                        iv_item_postselectionfragment_img,
                        R.drawable.icon_production_default);
            } else {
                iv_item_postselectionfragment_img
    ...
    public void refreshFlower(int position) {
        if (mDatas != null && mDatas.size() > 0) {
            ...
            List<GiftUsers> giftUsers = postsBean.getGiftUsers();
            if (giftUsers != null && giftUsers.size() > 0) {
                for (int i = 0; i < giftUsers.size(); i++) {
                    GiftUsers giftUsers1 = giftUsers.get(i);
                    if (giftUsers1 != null) {
                        if (giftUsers1.getId() == spUtil
                                .getInt("userid", 0)) {
                            return;
                        }
                    }
