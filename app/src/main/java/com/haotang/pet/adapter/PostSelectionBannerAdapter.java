...
public class PostSelectionBannerAdapter extends DynamicPagerAdapter {
    ...
    public PostSelectionBannerAdapter(Activity context) {
        super();
    ...
    public View getView(final ViewGroup container, final int position) {
        ...
        view.setLayoutParams(new ViewGroup.LayoutParams(
                ...
                ViewGroup.LayoutParams.MATCH_PARENT));
        final BannersBean bannersBean = list.get(position);
        if (bannersBean != null) {
            final String imgUrl = bannersBean.getImgUrl();
            final String imgLink = bannersBean.getImgLink();
            GlideUtil.loadImg(container.getContext(), imgUrl, view,
                    R.drawable.icon_production_default);
            view.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View arg0) {
                    if (bannersBean.getPoint() > 0) {
                        Utils.goService(context,bannersBean.getPoint(),bannersBean.getBackup());
                    } else {
