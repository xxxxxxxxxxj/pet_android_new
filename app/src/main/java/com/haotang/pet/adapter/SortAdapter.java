...
public class SortAdapter extends BaseAdapter implements SectionIndexer{
	...
	public View getView(final int position, View view, ViewGroup arg2) {
		...
		if (view == null) {
			...
			viewHolder.imageView_animal_icon = (ImageView) view.findViewById(R.id.imageView_animal_icon);
...
//		viewHolder.imageView_animal_icon.setTag(CommUtil.getServiceNobacklash()+list.get(position).getAvatarPath());
		...
		if(list.get(position).getAvatarPath()!=null&&!"".equals(list.get(position).getAvatarPath())){
			GlideUtil.loadCircleImg(mContext,list.get(position).getAvatarPath(),viewHolder.imageView_animal_icon,0);
		}
		return view;
	...
	final static class ViewHolder {
		TextView tvLetter;
		TextView tvTitle;
		ImageView imageView_animal_icon;
	}
