...
public class NodeProgressBar extends View implements Runnable {
     ...
     * 节点名称
     */
    private String[] textArr = new String[]{"下单支付", "邀请好友", "成功团购", "成功团购", "成功团购", "成功团购"};
    /**
     * 节点数
     */
     ...
     * 蓝色空心圆半径
     */
    private int r_blue = 76;
    /**
     * 白色进度条高度
     */
    private int whiteProgressHeight = 16;
    /**
     * 蓝色进度条高度
     */
     ...
     * 文本未激活颜色
     */
    private String textColorNotEnabled = "#666666";
    /**
     * 文本颜色j
     */
    private String numTextColor = "#FFFFFF";
    /**
     * 文本未激活颜色
     */
    private String numTextColorNotEnabled = "#666666";
    /**
     * View 背景颜色
     */
    ...
    public void draw(Canvas canvas) {
        ...
        for (int i = 0, j = list_whitecircle.size(); i < j; i++) {
            ...
            if (i < index) {
                paintNum.setColor(Color.parseColor(numTextColor));
            } else {
                paintNum.setColor(Color.parseColor(numTextColorNotEnabled));
            }
            float numTextWidht = paintNum.measureText(numStr);
            Log.e("TAG", "numTextWidht = " + numTextWidht);
            canvas.drawText(numStr, x - numTextWidht / 2, y + numTextWidht / 2, paintNum);
        }
        Log.e("TAG", "index = " + index);
        if (index == textArr.length) {
            BitmapDrawable drawableDuiGou = new BitmapDrawable(
                    BitmapFactory.decodeResource(getResources(),
                            R.drawable.progress_duigou_circle));
            drawCircle(canvas, drawableDuiGou,
                    maxProgressWidth / (count - 1) * (index - 1) + offAbs_x, r_white / 2 + offY, r_blue);
        }
