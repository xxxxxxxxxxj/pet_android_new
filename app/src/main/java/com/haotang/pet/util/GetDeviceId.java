...
//        return info.getMacAddress();
    ...
    public static void saveDeviceID(String str, Context context) {
        ...
        try {
            ...
            e.printStackTrace();
            if(!Utils.isStrNull(SharedPreferenceUtil.getInstance(context).getString("imei", ""))){
                SharedPreferenceUtil.getInstance(context).saveString("imei", str);
            }
    ...
    public static void saveUniqueId(Activity mContext) {
        if(Utils.isStrNull(readDeviceID(mContext))){
            return;
        }
        try {
            if (Utils.isStrNull(getIMIEStatus(mContext))) {
                Log.e("TAG", "tm.getDeviceId() = " + getIMIEStatus(mContext));
                GetDeviceId.saveDeviceID(getIMIEStatus(mContext), mContext);
            } else {
                String uuid = UUID.randomUUID().toString().replace("-", "");
                GetDeviceId.saveDeviceID(uuid, mContext);
                Log.e("TAG", "uuid = " + uuid);
            }
        } catch (Exception e) {
            Log.e("TAG", "saveUniqueId失败 e = " + e.toString());
            e.printStackTrace();
            String uuid = UUID.randomUUID().toString().replace("-", "");
            GetDeviceId.saveDeviceID(uuid, mContext);
            Log.e("TAG", "uuid = " + uuid);
