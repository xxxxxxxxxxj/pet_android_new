...
public class PetCircleFragment extends BaseFragment implements OnTouchListener {
    ...
    private void setView() {
        adapterBanner = new BannerBathLoopAdapter(context, rpvBanner, listBanner);
...
//		mDialog.showDialog();
    ...
    private AsyncHttpResponseHandler queryGroups = new AsyncHttpResponseHandler() {
        ...
        public void onFailure(int statusCode, Header[] headers,
            ...
            String pet_circle_data = spUtil.getString("PET_CIRCLE_DATA", "");
            if(Utils.isStrNull(pet_circle_data)){
                processData(pet_circle_data);
            }
            try {
                mDialog.closeDialog();
                pullToRefreshPetCircle.onRefreshComplete();
                        ...
                        if (arrayGroup.length() > 0) {
                            for (int i = 0; i < arrayGroup.length(); i++) {
                                ...
                                if (objectEvery.has("id") && !objectEvery.isNull("id")) {
                                    circle.PostGroupId = objectEvery.getInt("id");
                                }
                                if (objectEvery.has("extendParam")&&!objectEvery.isNull("extendParam")){
                                    JSONObject objectExtend = objectEvery.getJSONObject("extendParam");
                                    if (objectExtend.has("isFollowed")&&!objectExtend.isNull("isFollowed")){
                                        circle.isFollowed = objectExtend.getInt("isFollowed");
                                    }
                                }
...
//										listBanner.add(circleBanner.imgLink);
                                ...
                                if (objectBanner.has("point") && !objectBanner.isNull("point")) {
                                    circleBanner.point = objectBanner.getInt("point");
                                }
                                if (objectBanner.has("backup") && !objectBanner.isNull("backup")) {
                                    circleBanner.backup = objectBanner.getString("backup");
                                }
...
//						}
                    spUtil.saveString("PET_CIRCLE_DATA",result);
                }
            }
