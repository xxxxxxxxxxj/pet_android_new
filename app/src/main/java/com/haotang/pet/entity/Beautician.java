...
//	end ------------美容师详情页面预约数据
    ...
    public Beautician(int id, String name, int sort, String image) {
        this.id = id;
        this.name = name;
        this.sort = sort;
        this.image = image;
    ...
    public static Beautician json2Entity(JSONObject json) {
        ...
        try {
            if (json.has("extras") && !json.isNull("extras")) {
                JSONObject extras = json.getJSONObject("extras");
                if (extras.has("earliest") && !extras.isNull("earliest")) {
                    beautician.earliest = extras.getString("earliest");
                }
            }
            if (json.has("earliest") && !json.isNull("earliest")) {
                beautician.earliest = json.getString("earliest");
            }
            if (json.has("tid") && !json.isNull("tid")) {
                beautician.tid = json.getInt("tid");
            }
            ...
            if (json.has("avatar") && !json.isNull("avatar")) {
                beautician.image = json.getString("avatar");
            }
...
//			---------以下美容师详情页 点预约弹出信息
            ...
            if (json.has("icon") && !json.isNull("icon")) {
                beautician.BeauDetailIcon = json.getString("icon");
            }
            ...
            if (json.has("info") && !json.isNull("info")) {
                ...
                if (object.has("typeId") && !object.isNull("typeId")) {
                    beautician.BeauDetailTypeId = object.getInt("typeId");
                }
            }
            if (json.has("id") && !json.isNull("id")) {
                beautician.BeauDetailMenuItemId = json.getInt("id");
            }
//			-------------美容师是否支持上门、到店
