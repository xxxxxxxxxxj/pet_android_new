...
public class JCVideoPlayer extends FrameLayout implements View.OnClickListener, SeekBar.OnSeekBarChangeListener, SurfaceHolder.Callback, View.OnTouchListener, JCMediaManager.JCMediaPlayerListener {
    ...
    private TextView process_show_title;
    private JCVideoPlayer isFullScreen;
    private int type = 0;
    ...
    private void init(Context context) {
        ...
        process_show_title = (TextView) findViewById(R.id.process_show_title);
    ...
    public void setUp(String url, String title) {
        setUp(url, title, true,0);
        setPlaying(url);// 这个目前仅仅顶部banner 使用 可以这么直接写 如果 多界面 需要增加过滤条件
    }
     ...
     */
    public void setUp(String url, String title, boolean ifShowTitle,int type) {
    	this.type=type;
...
//    }
    ...
    public static void setPause(int CURRENT_STATE){
    	CURRENT_STATE = CURRENT_STATE_PAUSE;
        JCMediaManager.intance().mediaPlayer.pause();
        Log.i("JCVideoPlayer", "pause video");
    }
    
    public void setPlaying(String url){//进入自动播放
    	setState(CURRENT_STATE);
    	ivStart.performClick();
    }
}
