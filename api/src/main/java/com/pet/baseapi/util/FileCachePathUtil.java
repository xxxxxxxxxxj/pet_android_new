...
/**
 * Created by zarkshao on 2017/1/18.
 */
...
public class FileCachePathUtil {
    ...
    public static File getExternalCacheDirectory(Context context, String type) {
        ...
        if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())) {
            if (TextUtils.isEmpty(type)) {
                appCacheDir = context.getExternalCacheDir();//存放临时数据
            } else {
                appCacheDir = context.getExternalFilesDir(type);//长时间保存的数据
            }
    ...
    public static File getCacheImageFile(Context context, String fileName) {
        return new File(getExternalCacheDirectory(context, IMAGE_CACHE).getAbsolutePath() + "/" + fileName + ".png");
    }

    public static File getCacheNetWorkCache(Context context, String fileName) {
        return new File(getExternalCacheDirectory(context, CACHE_NETWORK), fileName);
    }
